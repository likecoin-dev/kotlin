...
public class BuiltInsSerializer(private val dependOnOldBuiltIns: Boolean) {
    ...
    private fun getFileName(classDescriptor: ClassDescriptor): String {
        return BuiltInsSerializationUtil.getClassMetadataPath(getClassId(classDescriptor))
    }

    private  fun getClassId(descriptor: ClassDescriptor): ClassId {
        val owner = descriptor.getContainingDeclaration()
        if (owner is PackageFragmentDescriptor) {
            return ClassId(owner.fqName, descriptor.getName())
        }
        return getClassId(owner as ClassDescriptor).createNestedClassId(descriptor.getName())
    }
}
